...
*       v1.4: Bug fixes, Chargers can be placed against any surface, Supports ROLL rotation.
*
*/
...
enum _:MAIN_SETTINGS
{
    ...
    SETTING_HEALTH_REFILL[MAX_RESOURCE_PATH_LENGTH],
    SETTING_HEALTH_REMOVED[MAX_RESOURCE_PATH_LENGTH],
    ...
    SETTING_HEV_REFILL[MAX_RESOURCE_PATH_LENGTH],
    SETTING_HEV_REMOVED[MAX_RESOURCE_PATH_LENGTH],
...
public plugin_init()
{
    ...
    RegisterHam(Ham_TraceAttack, "info_target", "fwdTraceAttack", 0)
            ...
            default:
            {
                switch( iSection )
                {
                    ...
                    case SECTION_MAIN_SETTINGS:
                    {
                        ...
                        else if ( equali(szKey, "SETTING_HEALTH_REFILL") )
                        {
                            copy(g_eSettings[SETTING_HEALTH_REFILL], charsmax(g_eSettings[SETTING_HEALTH_REFILL]), szValue)
                            if ( !g_bFileWasRead ) precache_sound(g_eSettings[SETTING_HEALTH_REFILL])
                        }
                        else if ( equali(szKey, "SETTING_HEALTH_REMOVED") )
                        {
                            copy(g_eSettings[SETTING_HEALTH_REMOVED], charsmax(g_eSettings[SETTING_HEALTH_REMOVED]), szValue)
                            if ( !g_bFileWasRead ) precache_sound(g_eSettings[SETTING_HEALTH_REMOVED])
                        }
                        ...
                        else if ( equali(szKey, "SETTING_HEV_REFILL") )
                        {
                            copy(g_eSettings[SETTING_HEV_REFILL], charsmax(g_eSettings[SETTING_HEV_REFILL]), szValue)
                            if ( !g_bFileWasRead ) precache_sound(g_eSettings[SETTING_HEV_REFILL])
                        }
                        else if ( equali(szKey, "SETTING_HEV_REMOVED") )
                        {
                            copy(g_eSettings[SETTING_HEV_REMOVED], charsmax(g_eSettings[SETTING_HEV_REMOVED]), szValue)
                            if ( !g_bFileWasRead ) precache_sound(g_eSettings[SETTING_HEV_REMOVED])
                        }
...
public chargerTask()
{
    ...
    for ( new i = 0; i < iNum; i ++ )
    {
        ...
        if ( chargerTrace(id) ) set_rendering(iEnt, kRenderFxNone, 255, 255, 255, kRenderNormal, 255)
        else                    set_rendering(iEnt, kRenderFxNone, 255, 255, 255, kRenderTransAlpha, g_eSettings[SETTING_GHOST_ALPHA])
    }
...
public fwdTraceAttack(iEnt, iAttacker, Float:fDamage, Float:fDirection[3], iTr, iDamageBits)
{
    ...
    chargerBulletParticles(fEnd)
    chargerBulletSpark(fEnd)
...
stock chargerSetGlow(iEnt, bool:bGlow)
{
    ...
    if ( bGlow ) set_rendering(iEnt, kRenderFxGlowShell, 255, 0, 0, kRenderTransAlpha, 32)
    ...
    switch( iSound )
    {
        case SOUND_HEALTH:
        {
            switch( iEvent )
            {
                ...
                case EVENT_REFILL:  copy(szSample, charsmax(szSample), g_eSettings[SETTING_HEALTH_REFILL])
                case EVENT_REMOVED: copy(szSample, charsmax(szSample), g_eSettings[SETTING_HEALTH_REMOVED])
            }
        }
        case SOUND_HEV:
        {
            switch( iEvent )
            {
                ...
                case EVENT_REFILL:  copy(szSample, charsmax(szSample), g_eSettings[SETTING_HEV_REFILL])
                case EVENT_REMOVED: copy(szSample, charsmax(szSample), g_eSettings[SETTING_HEV_REMOVED])
            }
        }
    }
...
stock chargerBulletParticles(Float:fOrigin[3])
{
    message_begin_f(MSG_PVS, SVC_TEMPENTITY, fOrigin)
...
stock chargerBulletSpark(Float:fOrigin[3])
{
    message_begin_f(MSG_PVS, SVC_TEMPENTITY, fOrigin)
