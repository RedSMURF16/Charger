    ...
    switch( item )
    {
        ...
        case ROTATE_PLACE:
        {
            if ( chargerTrace(id) )
            {
                ...
                g_ePlayerData[id][PDATA_NEXT_USE]      = get_gametime() + 0.25
...
public fwdPreThink(id)
{
    ...
    else
    {
        ...
        {
            if ( get_gametime() > g_ePlayerData[id][PDATA_NEXT_USE] )
...
chargerSupplyHealth(id, eCharger[CHARGER])
{
    ...
    g_ePlayerData[id][PDATA_NEXT_USE] = get_gametime() + eCharger[CHARGER_COOLDOWN]
...
stock playerReset(id)
{
    ...
    g_ePlayerData[id][PDATA_NEXT_USE]    = 0.0
